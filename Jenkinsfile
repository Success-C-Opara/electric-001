    ...
    stages {
        ...
        stage('Build Docker Image') {
            steps {
                ...
                bat """
                bash -c "docker build -t ${IMAGE_NAME}:${IMAGE_TAG} ."
                """
            }
        }
        ...
        stage('Save Docker Image') {
            steps {
                ...
                bat """
                bash -c "docker save -o ${IMAGE_NAME}-${IMAGE_TAG}.tar ${IMAGE_NAME}:${IMAGE_TAG}"
                """
        ...
        stage('Copy and Deploy to EC2') {
            steps {
                ...
                bat """
                bash -c "scp -o StrictHostKeyChecking=no -i '${PRIVATE_KEY_PATH}' ${IMAGE_NAME}-${IMAGE_TAG}.tar ${EC2_USER}@${EC2_HOST}:~/"

                bash -c "ssh -o StrictHostKeyChecking=no -i '${PRIVATE_KEY_PATH}' ${EC2_USER}@${EC2_HOST} \\
